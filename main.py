...
from source.walls import Walls
from source.snake import Snake
from source.apple import *
...
pygame.display.set_caption('Old_snake')
...
def draw_text():
    text = "Яблоки:{} Очки: {} Жизни: {} ".format(apple.count, hero.points, "-" * hero.lives)
...
def popup(msg):
    ...
    entry.pack()
    entry.insert(0, 'Аноним')  # default name
    entry.bind("<Return>", set_name)
    entry.focus_set()

    b1 = Button(popupwin, text="Хорошо", command=set_name)
    b1.pack()
...
def main():
    ...
    while True:  # main loop
        ...
        if hero.hit_walls(walls_list):  # check if snake hits the walls or itself
            ...
            if hero.lives <= 0:
                game_over = True
                print_text(LARGE_FONT, "КОНЕЦ", RED)
